...
def filter_ordinal_function(scene_struct, obj_idxs, o_num, o_dir):
  ...
  #print(objs_coords)
  #print("JIJI")
  #for x in objs_coords:
  #    print(x[1][1])
  #print("tito")
  left_to_right = sorted(objs_coords, key = lambda x: x[1][0])
  front_to_end = sorted(objs_coords, key = lambda x: x[1][1])
  #front_to_end = sorted(objs_coords, key = lambda x: x[1][2])  # wrong
...
def get_visible_occlusion_sets(scene_struct, thre1, thre2):
  ...
  obj_bbox = [(idx, 
               ...
               scene_struct['objects'][idx]['pixel_coords'][1]) for idx,_ in objs]  # changed pixel coords to 1, theres no 2
  ...
  for i in range(len(obj_bbox)):
    ...
    for j in range(i+1, len(obj_bbox)):
      ...
      #assert iz != jz  # does this assertion help at all?
      #print("Jiji")
